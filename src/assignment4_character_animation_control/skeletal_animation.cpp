...
int main()
{
	...
	Model ourModel(FileSystem::getPath("resources/objects/mixamo/granny/Sporty Granny.dae"));
	Animation idleAnimation(FileSystem::getPath("resources/objects/mixamo/Idle/Idle.dae"), &ourModel);
	Animation walkAnimation(FileSystem::getPath("resources/objects/mixamo/Walking/Walking.dae"), &ourModel);
	Animation runAnimation(FileSystem::getPath("resources/objects/mixamo/Run/Run.dae"), &ourModel);
	Animation punchAnimation(FileSystem::getPath("resources/objects/mixamo/Punch Combo/Punch Combo.dae"), &ourModel);
	Animation kickAnimation(FileSystem::getPath("resources/objects/mixamo/Side Kick/Side Kick.dae"), &ourModel);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS && charState != IDLE_PUNCH && charState != PUNCH && charState != PUNCH_IDLE)
		{
			blendAmount = 0.0f;
		...
		case RUN:
			...
			if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS)
			{
				...
				if (blendAmount > 0.9f)
				{
					blendAmount = 0.0f;
					float startTime = animator.m_CurrentTime2;
					animator.PlayAnimation(&walkAnimation, &punchAnimation, startTime, 0.0f, blendAmount);
					charState = WALK_PUNCH;
				}
			}
			if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS)
			{
				...
				if (blendAmount > 0.9f)
				{
					blendAmount = 0.0f;
					float startTime = animator.m_CurrentTime2;
					animator.PlayAnimation(&walkAnimation, &kickAnimation, startTime, 0.0f, blendAmount);
					charState = WALK_KICK;
				}
			}
			printf("running\n");
		...
		case IDLE_PUNCH:
			...
			break;
		case PUNCH:
			animator.PlayAnimation(&punchAnimation, NULL, animator.m_CurrentTime, animator.m_CurrentTime2, blendAmount);
			{
				float punchDuration = punchAnimation.GetDuration();
				if (punchDuration > 0.0f && animator.m_CurrentTime > punchDuration * 0.7f)
				{
					blendAmount = 0.0f;
					animator.PlayAnimation(&punchAnimation, &idleAnimation, animator.m_CurrentTime, 0.0f, blendAmount);
		...
		case IDLE_KICK:
			...
			break;
		case KICK:
			animator.PlayAnimation(&kickAnimation, NULL, animator.m_CurrentTime, animator.m_CurrentTime2, blendAmount);
			{
				float kickDuration = kickAnimation.GetDuration();
				if (kickDuration > 0.0f && animator.m_CurrentTime > kickDuration * 0.7f)
				{
					blendAmount = 0.0f;
					animator.PlayAnimation(&kickAnimation, &idleAnimation, animator.m_CurrentTime, 0.0f, blendAmount);
					charState = KICK_IDLE;
				}
			}
			printf("kicking \n");
